...
const _createSemester = async (name, batch) => {
    const semester = await new Semester({name, batch}).save();
    return semester._id;
};
...
const addBatch = async (req, res, next) => {
    try {
        ...
        const referenceSemester = await Batch.updateOne({_id: batch._id}, {
            $push: {
                semesters: semesters
            }
        });
        ...
        if (referenceSemester.nModified <= 0) {
            await Semester.deleteOne({_id: semesters[0]});
            await Semester.deleteOne({_id: semesters[1]});
            return next(error(500, "Unable to reference Semester"));
        }

        const referenceBatch = await Department.updateOne({head: req.user._id}, {
            $push: {
                batches: batch._id,
            }
        });
...
const deleteBatch = async (req, res, next) => {
    try {
        const response = await Batch.deleteOne({_id: req.params.id});
...
const updateBatch = async (req, res, next) => {
    try {
        const response = await Batch.updateOne({_id: req.params.id}, {$set: req.body});
